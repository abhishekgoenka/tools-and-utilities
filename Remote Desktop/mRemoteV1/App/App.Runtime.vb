...
#Region "Opening Connection"
        ...
        Public Shared Sub OpenSQLConnection()
            Try
                If Windows.treeForm.tvConnections.SelectedNode.Tag Is Nothing Then
                    Exit Sub
                End If

                Dim connectionInfo As New mRemoteNG.Connection.Info
                connectionInfo = Windows.treeForm.tvConnections.SelectedNode.Tag

                Dim param As String
                param = String.Format(" -s {0} -d works -u {1} -p {2} -nosplash", connectionInfo.SQLServerIP, connectionInfo.SQLServerUserName, connectionInfo.SQLServerPassword)
                Process.Start("ssms", param)
        ...
        Public Shared Sub OpenSQLProfiler()
            Try
                If Windows.treeForm.tvConnections.SelectedNode.Tag Is Nothing Then
                    Exit Sub
                End If
                ...
                Dim param As String
                param = String.Format(" -server {0} -user {1} -password {2}", connectionInfo.SQLServerIP, connectionInfo.SQLServerUserName, connectionInfo.SQLServerPassword)
                Process.Start("C:\Windows\expressprofiler.exe", param)
